...
void AppFriday::handleInputs(uint32_t nowMs)
{
    ...
    if ((_presence_state == PresenceState::Host || _presence_state == PresenceState::Departing ||
         _presence_state == PresenceState::AwaitingReturn) &&
        (GetHAL().btnA.wasPressed() || GetHAL().btnB.wasPressed())) {
    ...
    // Friday reacts on the debounced press edge instead of waiting for the
    // button-release click generated by KeyManager. This makes the physical
    // corner feel as if it touched the face at the exact moment it moves.
    if (GetHAL().btnA.wasPressed() && GetHAL().btnB.isReleased()) {
        _face_model.react(friday::Reaction::ButtonA, nowMs, -0.84f, -0.78f);
    } else if (GetHAL().btnB.wasPressed() && GetHAL().btnA.isReleased()) {
    ...
    switch (keyEvent) {
        ...
        case input::KeyEvent::GoNext:
            // The corresponding animation already started on press. Ignore
            // the release click so one push cannot replay it a second time.
            break;
        default:
            break;
    }
