...
- 左上留白显示设备名称和 `USB` / `BLE` / `Offline`，点击可打开设备列表。
  沿用原界面的灰色小字与状态点，不占用独立胶囊；A1 恢复与其他按键相同尺寸。
...
- 实际 LVGL 指针交互验证 A1、设备入口、菜单隔离与滚动；长名称单行省略，
  `Offline` 独立可读，圆屏留白不裁字。保护测试只更新已验证 UI 的指纹基线。
- 包含 UI 调整的候选 4,365,952 字节，SHA-256：
  `e90bbfaff02c57fb885decb01b33dcec77d7ef1b8c91060738d1826453d97d85`；尚未刷入。
