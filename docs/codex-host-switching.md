...
- 仅调整 Codex USB 配置，保留 Report 6 的 63 字节双向协议、Feature 7/8 的
  128/256 字节载荷；SDK、Friday、其他应用和桌面软件未修改。
- 完整构建与原有 13 组主机检查通过。新增 ELF 描述符检查直接解析链接后的字节：
  旧 `.7` 因中断 IN/OUT 两个端点失败，新 `.8` 仅 `0x81` IN、EP0 写入的配置通过。
- 候选 4,366,000 字节，SHA-256：
  `66147e80e41feba97d6827e097d32ef557d370d6d0cfe7d199c47e10d9da5bcd`；尚未刷入。
